...
- special functions:
   ...
   - local scoped variables:
      ...
      - each local scope of each variable also has its own private stack. This allows additional dynamic scoping in [Braceplate message sequences](#braceplate-message-sequences). The stack (`&push^x`, `&pop^x` to push/pop variable `x`) can also be used as a queue (`&shift^x`, `&unshift^x`). You know, it's kind of a hack. Just forget you ever read this bullet, it's dangerous knowledge that could hurt those close to you
   - repetition:
